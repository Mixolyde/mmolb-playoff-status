...
void main() {
  ...
  getContentPages().then((v) {
    ...
    cron.schedule(
      Schedule.parse('1,21,26,31,36,41,46,51,56 * * * *'),
      () async {
        if (!document.hidden && currentView.activeView != View.about) {
          await refreshData();
        }
      },
    );
...
Future<void> refreshData() async {
  ...
  var standingsTable =
      document.querySelector('#standingsTable') as HTMLTableElement?;
  ...
  switch (currentView.activeView) {
    case View.winsbehind:
      populateGamesBehindTable(
        allStandings,
        sitedata,
        currentView.groupBySubLeague,
      );
      break;
    case View.winningmagic:
      populateWinningTable(
        allStandings,
        sitedata,
        currentView.groupBySubLeague,
      );
...
void addListeners() {
  ...
  document
      .querySelector('#viewEliminationNumbers')!
      .onClick
      .listen(selectViewE);
  document.querySelector('#viewAbout')!.onClick.listen(selectViewAbout);
  document
      .querySelector('#viewPostseasonChances')!
      .onClick
      .listen(selectViewPost);
...
void selectViewButton() {
  switch (currentView.activeView) {
    case View.about:
      document.querySelector('#viewAbout')!.classList.add('nav-button-active');
      document
          .querySelector('#viewChances')!
          .classList
          .remove('nav-button-active');
      document
          .querySelector('#viewWinsBehind')!
          .classList
          .remove('nav-button-active');
      document
          .querySelector('#viewWinningNumbers')!
          .classList
          .remove('nav-button-active');
      document
          .querySelector('#viewEliminationNumbers')!
...
void handlePopState(PopStateEvent event) {
  print(
    "PopStateEvent: ${event.toString()} ${event.type.toString()} ${event.state.toString()} ${event.state.runtimeType} ",
  );
  if (event.state != null) {
...
void pushViewState() {
  ...
  print(
    'Pushing View State: ${currentView.toHash().toJS} ${currentView.toHash().toJS.runtimeType} ${currentView.toJson()}',
  );
  window.history.pushState(currentView.toHash().toJS, '', currentView.toHash());
}

void replaceViewState() {
  //update URL with popstate
  print(
    'Replacing View State: ${currentView.toJson().toJSBox} ${currentView.toJson()}',
  );
  window.history.replaceState(
    currentView.toHash().toJS,
    '',
    currentView.toHash(),
  );
}
...
CurrentView loadCurrentView() {
  ...
  if (window.localStorage.getItem('current_view') != null) {
    return CurrentView.fromJson(
      json.decode(window.localStorage.getItem('current_view')!),
    );
  } else {
